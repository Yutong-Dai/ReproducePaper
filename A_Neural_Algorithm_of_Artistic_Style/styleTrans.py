...
Last Modified: 2021-03-21 23:11
--------------------------------------------
Description:
'''
import torch
...
def style_transfer(c='./db/pikachu.jpg', s='./db/pikachu.jpg', epochs=400,
                   ...
                   optimizer='adam', lr=0.1, scheduler='none'):
    # load model
    ...
    STYLE_LAYERS_WEIGHTS = [layer_weights for i in range(16)]
    for _ in range(epochs):
        optimizer.zero_grad()
        imgInput = torch.transpose(G.unsqueeze(0), 1, 3)
        aGs = utils.get_feature_maps(imgInput, layers)
        loss = utils.compute_total_cost(aGs, aCs, aSs, STYLE_LAYERS_WEIGHTS,
                                        content_layer_idx=c_layer, alpha=1, beta=6)
        loss.backward()
        optimizer.step()
        scheduler.step()
    return G.cpu()
